...
fn path_shaped_arguments_that_name_no_file_are_reported() {
    ...
    // missing scripts, not expressions. The absolute path lives under a
    // tempdir and is never created, so the test never depends on
    // whether this machine has epher's scripts installed.
    let dir = tempfile::tempdir().unwrap();
    let missing = dir.path().join("astronomy/moon/full-moons.epher");
    for path in [
        missing.display().to_string(),
        "./sine.es".to_string(),
        "../sine.es".to_string(),
        "C:\\Users\\pete\\scripts\\full-moons.epher".to_string(),
        "C:/Users/pete/scripts/full-moons.epher".to_string(),
        "\\\\server\\share\\sine.es".to_string(),
    ] {
        assert_eq!(
            parse(&[path.as_str()]),
            Action::MissingScriptFile(path.clone()),
            "{path:?}"
        );
    }
}
