...
impl Bridge {
    ...
    pub fn open_external(self, url: &str) {
        let args = serde_wasm_bindgen::to_value(&OpenUrlArgs { url }).unwrap_or(JsValue::UNDEFINED);
        self.spawn("open_url", args);
    }

    /// File → Save script (ADR-0024): the desktop shell's native
