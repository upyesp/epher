...
static TABS: &[TabDef] = &[
    ...
    TabDef {
        ...
        keys: &[
            ...
            key("date", KeyAction::Call("date"), "fn", "key-hint-date"),
            key("time", KeyAction::Call("time"), "fn", "key-hint-time"),
            key("iso", KeyAction::Call("iso"), "fn", "key-hint-iso"),
...
/// event (`frame_armed` / `frame_handle` track that frame). `vel` is
/// the frame-to-frame velocity the release snap reads.
struct KeypadDrag {
    pointer_id: i32,
    y0: f64,
    start_h: f64,
...
/// fills; `fill` stays a CSS property so the palette follows the theme.
fn brand_icon() -> yew::Html {
    yew::Html::from_html_unchecked(
        r#"<svg class="brand-icon" viewBox="0 0 512 512" aria-hidden="true" focusable="false">
  <rect class="brand-tile" width="512" height="512" rx="96"/>
  <g transform="translate(38.55 57.35) scale(4.85)">
    <path class="brand-glyph" d="M66 10C72.2882 25.3312 78.7226 54.8252 54 57L54 2C36.7894 2 19.7138 2.46282 8.63812 18C3.99588 24.5122 3.00299 32.2411 3 40C2.99172 61.4514 16.8873 82.2066 40 83.9097C56.9179 85.1564 74.8247 82.3137 84.892 67C92.498 55.4303 92.6651 37.173 89.9961 24C89.1789 19.9666 88.3893 13.4681 84.6065 11.0278C80.2558 8.2212 71.0315 10 66 10z"/>
    <path class="brand-counter" d="M38 57L38 27C33.573 27.193 28.8439 27.8014 25.3403 30.8032C13.3927 41.0398 26.0822 55.9516 38 57z"/>
  </g>
</svg>"#.into(),
    )
}
    ...
    spawn_local(async move {
        let report =
            |result: &UseStateHandle<String>, localizer: &UseStateHandle<Localizer>, name: &str| {
                result.set(localizer.lookup_args("saved-script", &[("name", name)]));
            };
        if bridge == Bridge::Tauri {
...
fn epher_app() -> Html {
    ...
    let keypad_animate = {
        ...
        move |open: bool, dur_ms: Option<i32>| {
            ...
            // The snap's own pace, matched to the flick. The shorthand
            // (not just the duration) goes inline so the closing
            ...
            // drag's `transition: none` class rule applies again.
            *keypad_anim.borrow_mut() =
                Some(gloo_timers::callback::Timeout::new(timeout_ms, move || {
                    let _ = clip.style().remove_property("height");
                    let _ = clip.style().remove_property("transition");
                }));
        }
    };
    let on_grab_down = {
    ...
    let on_grab_move = {
        ...
        Callback::from(move |e: web_sys::PointerEvent| {
            ...
            // one style write a frame, not one per event, and the
            // drag stays glued to the pointer without layout storms.
            d.pending_y = Some(y);
            if !d.frame_armed {
                let drawer = keypad_drawer_ref.clone();
                let drag_cell = keypad_drag.clone();
                let frame = Closure::wrap(Box::new(move || {
                    let mut drag = drag_cell.borrow_mut();
                    let Some(d) = drag.as_mut() else {
                        return;
                    };
                    d.frame_armed = false;
                    let Some(y) = d.pending_y.take() else {
                        return;
                    };
                    // Frame-to-frame velocity (EMA-smoothed), the
                    // release flick's read on how fast the drawer
                    // was really moving.
                    let t = js_sys::Date::now();
                    let dt = (t - d.last_t) as f64;
                    if dt > 0.0 {
                        d.vel = 0.7 * ((y - d.last_y) / dt) + 0.3 * d.vel;
                    }
                    d.last_y = y;
                    d.last_t = t;
                    // Dragging down shrinks the drawer; the history
                    // list grows into the freed space live.
                    let h = (d.start_h - (y - d.y0)).clamp(0.0, d.open_h);
                    if let Some(drawer) = drawer.cast::<Element>() {
                ...
                if let Some(w) = web_sys::window() {
                    if let Ok(handle) = w.request_animation_frame(frame.as_ref().unchecked_ref()) {
                        let d = drag.as_mut().unwrap();
                        d.frame_armed = true;
                        d.frame_handle = handle;
                        std::mem::forget(frame); // fired once; the flags above govern it
                    }
                }
            }
        })
    ...
    let on_grab_end = {
        ...
        Callback::from(move |e: web_sys::PointerEvent| {
            ...
            if d.frame_armed {
                if let Some(w) = web_sys::window() {
                    let _ = w.cancel_animation_frame(d.frame_handle);
                }
            }
            ...
            *keypad_last_gesture.borrow_mut() = js_sys::Date::now();
            ...
            let current = keypad_drawer_ref
                ...
                .unwrap_or(0.0);
            let collapse = keypad_snap(current, d.open_h, d.vel);
            let open = !collapse;
    ...
    *menu_open_cell.borrow_mut() = *menu_open;
    ...
    let on_brand_click = {
        ...
        Callback::from(move |e: web_sys::MouseEvent| {
            if bridge == Bridge::Tauri {
                e.prevent_default();
                bridge.open_external(BRAND_URL);
            }
        })
    };
    ...
    html! {
        <main class="epher">
            ...
            <header class="topbar">
                ...
                // The brand on the mobile top line: the mark beside the
                // word, to the left of the pane switch so Calculator
                // stays the centre link of the three. Hidden at the
                // desktop breakpoint, where the rail's own brand item
                // takes over.
                <a class="brand-link" href={BRAND_URL} onclick={{
                    let close_hamburger = close_hamburger.clone();
                    let on_brand_click = on_brand_click.clone();
                    Callback::from(move |e: web_sys::MouseEvent| {
                        close_hamburger.emit(());
                        on_brand_click.emit(e);
                    })
                }}>
                    { brand_icon() }
                    <span>{"epher"}</span>
                ...
                >
                    // The rail's brand item at the top of the vertical
                    // menu: the mark over the word, linking to the
                    // project site.
                    <a
                        class="menu-brand"
                        role="menuitem"
                    ...
                    >
                        { brand_icon() }
                        <span class="menu-brand-name">{"epher"}</span>
                    </a>
                    <div class="menu">
                        <button
                            type="button"
