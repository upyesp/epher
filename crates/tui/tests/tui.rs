...
fn keypad_digits_bank_mirrors_the_web_tab() {
    ...
    app.keypad_set(2, 2); // % sits mid-grid now that random keys joined
    app.keypad_insert();
...
fn keypad_banks_cycle_and_reset_the_highlight() {
    ...
    assert_eq!(app.keypad_bank(), "astro");
    app.keypad_insert(); // (0,0) of astro = jd
    assert_eq!(app.input(), "jd(");
    // var keeps its spot in the middle of the pack (web tab order)
    app.keypad_select_bank(8);
    assert_eq!(app.keypad_bank(), "var");
    app.keypad_insert(); // (0,0) of var = pi
    assert_eq!(app.input(), "jd(pi");
}
...
fn keypad_has_the_graph_commands() {
    let mut app = App::default();
    app.keypad_open();
    app.keypad_select_bank(8); // var: web tab order puts it after 0x
...
fn prompt_prefill_edits_at_the_caret() {
    ...
    assert_eq!(app.prompt_caret(), "epher-script.epher".len());
    // Backspace at the end drops the trailing 'r'.
    app.prompt_pop();
    assert_eq!(
        app.prompt_active(),
        Some((epher_tui::PromptKind::SaveScript, "epher-script.ephe"))
    );
    // Left, then a typed character inserts mid-word, not at the end.
    app.prompt_nudge(-1);
    app.prompt_push('H');
    assert_eq!(
        app.prompt_active(),
        Some((epher_tui::PromptKind::SaveScript, "epher-script.ephHe"))
    );
    // Home jumps to the start; typing prefixes the whole path.
    app.prompt_edge(-1);
    app.prompt_push('/');
    assert!(app
        .prompt_active()
        .is_some_and(|(_, b)| b.starts_with("/epher-script.ephHe")));
    // End, one step back, then Delete removes the character the caret
    // sits on (at the very end there is nothing under it).
    app.prompt_edge(1);
    app.prompt_nudge(-1);
    app.prompt_delete();
    assert!(app
        .prompt_active()
        .is_some_and(|(_, b)| b.ends_with("ephH")));
    // A click (display column) places the caret, and Backspace deletes
    // before it: the path now starts "/epher…", so a click on column 2
    // (past the leading "/" and "e") drops the "e".
    app.prompt_click(2);
    app.prompt_pop();
    assert!(app
        .prompt_active()
        .is_some_and(|(_, b)| b.starts_with("/pher-script.ephH")));
}
...
fn file_prompt_saves_and_opens() {
    ...
    assert_eq!(
        app.prompt_active(),
        Some((epher_tui::PromptKind::SaveScript, "epher-script.epher"))
    );
...
// ===== The history file prompts are gone (ADR-0027 amendment): the
// share icon moves history between devices, so Load/Save history no
// longer exist as file operations anywhere. =====

#[test]
fn multiline_history_entries_pick_as_one_semicolon_joined_line() {
...
fn mouse_keypad_clicks_select_banks_and_cells() {
    ...
    let expected = epher_tui::banks()[8].1[1][2].1;
    app.keypad_set(1, 2);
    app.keypad_insert();
...
#[test]
fn menu_toggle_flips_the_menu_open_state() {
    let mut app = App::with_session(epher_core::Session::new());
    assert_eq!(app.menu_active(), None);
    app.menu_toggle();
    assert_eq!(app.menu_active(), Some((0, 0)));
