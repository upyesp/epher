...
fn exact_str(name: &str, text: &str, expected: &str) {
    match epher_core::evaluate(text) {
        Ok(epher_core::Value::Str(s)) => assert_eq!(s, expected, "{name}"),
        other => panic!("{name} produced {other:?}"),
    }
}

#[test]
fn date_time_and_iso_read_julian_dates_back() {
    // round-trip: date() inverts jd() on the date part
    match epher_core::evaluate("date(jd(2026, 3, 20, 12))") {
        Ok(epher_core::Value::List(v)) => {
            let nums: Vec<f64> = v
                .into_iter()
                .map(|x| match x {
                    epher_core::Value::Float(f) => f,
                    other => panic!("date part {other:?}"),
                })
                .collect();
            assert_eq!(nums, vec![2026.0, 3.0, 20.0]);
        }
        other => panic!("date produced {other:?}"),
    }
    // the epoch: JD 0 is -4712 Jan 1 12:00 (astronomical year numbering)
    exact_str("epoch", "iso(0)", "-4712-01-01T12:00:00");
    // J2000: 2000-01-01 12:00 UT
    exact_str("J2000", "iso(jd(2000, 1, 1, 12))", "2000-01-01T12:00:00");
    exact_str(
        "fractional day",
        "iso(jd(1900, 6, 15, 18.5))",
        "1900-06-15T18:30:00",
    );
    // the Gregorian switch: Thursday 1582-10-04 (Julian) is followed by
    // Friday 1582-10-15 (Gregorian); JD 2299160.5 is the boundary
    match epher_core::evaluate("date(jd(1582, 10, 15))") {
        Ok(epher_core::Value::List(v)) => assert_eq!(v[0], epher_core::Value::Float(1582.0)),
        other => panic!("date produced {other:?}"),
    }
    exact_str(
        "last Julian day",
    ...
    // the pair is a true inverse across the switch: jd() and date()
    // agree both sides of 1582-10-15
    assert_eq!(
        eval_str("date(jd(1582, 10, 4, 12))[1]"),
        Value::Float(1582.0)
    );
    assert_eq!(eval_str("date(jd(1582, 10, 4, 12))[2]"), Value::Float(10.0));
    assert_eq!(eval_str("date(jd(1582, 10, 4, 12))[3]"), Value::Float(4.0));
    // time() rounds to the nearest second; the 24:00:00 carry becomes
    // the next day in iso() and wraps in time()
    exact_str(
        "seconds round",
        "iso(jd(2026, 3, 20) + 12.1291872002 / 24)",
        "2026-03-20T12:07:45",
    );
    exact_str(
        "carry into the next day",
        "iso(jd(2026, 12, 21) + (86400 - 0.4) / 86400)",
        "2026-12-22T00:00:00",
    );
    // date() never borrows from the next day: the instant still sits
    // inside 2026-12-21
    match epher_core::evaluate("date(jd(2026, 12, 21) + 0.99999)") {
        Ok(epher_core::Value::List(v)) => {
            assert_eq!(v[1], epher_core::Value::Float(12.0));
            assert_eq!(v[2], epher_core::Value::Float(21.0));
        }
        other => panic!("date produced {other:?}"),
    }
    // the domain floor: the recipe reads day numbers from 0 up
    assert!(epher_core::evaluate("date(-100)").is_err());
    assert!(epher_core::evaluate("date(\"x\")").is_err());
}
