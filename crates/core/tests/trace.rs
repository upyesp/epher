...
use epher_core::{eval_with_trace, Session, StatementOutcome};
...
fn every_statement_records_what_it_produced() {
    ...
    let outcomes = eval_with_trace(&mut session, "x = 40 + 2\nx + 1");
...
fn statements_without_a_value_produce_no_display() {
    let mut session = Session::default();
    let outcomes = eval_with_trace(&mut session, "def f(a) = a * 2\nf(21)");
...
fn state_flows_across_statements() {
    let mut session = Session::default();
    let outcomes = eval_with_trace(&mut session, "base = 2\nheight = 5\nbase * height / 2");
...
fn the_first_error_ends_the_pass() {
    let mut session = Session::default();
    let outcomes = eval_with_trace(&mut session, "x = 1 / 0\ny = 2");
...
fn definitions_reach_the_session() {
    let mut session = Session::default();
    eval_with_trace(&mut session, "def double(a) = a * 2\nconst tax = 21\nv = double(3)");
...
fn defined_names_surface_for_completion() {
    let mut session = Session::default();
    eval_with_trace(&mut session, "alpha = 1\nbeta = 2\ndef gem(a) = a\nconst k = 9");
