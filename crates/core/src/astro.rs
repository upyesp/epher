...
pub(crate) fn call(name: &str, args: Vec<Value>) -> Option<Result<Value, EpherError>> {
    let result = match name {
        ...
        "date" => one_float(name, &args).and_then(|jd| {
            jd_to_ymdhms(name, jd, Round::Floor).map(|(y, m, d, _, _, _)| {
                Value::List(vec![
        ...
        "time" => one_float(name, &args).and_then(|jd| {
            jd_to_ymdhms(name, jd, Round::Wrap).map(|(_, _, _, h, mi, s)| {
                Value::List(vec![
                    Value::Float(h as f64),
                    Value::Float(mi as f64),
                    Value::Float(s as f64),
                ])
            })
        }),
        "iso" => one_float(name, &args).and_then(|jd| {
            jd_to_ymdhms(name, jd, Round::Carry)
                .map(|(y, m, d, h, mi, s)| Value::Str(iso_string(y, m, d, h, mi, s)))
        }),
        "now" => now(&args),
...
/// arithmetic is Meeus's full recipe (Astronomical Algorithms ch. 7):
/// dates from 1582-10-15 onward are Gregorian and take the century
/// correction `b`; earlier dates are Julian calendar and take `b = 0` --
/// the inverse of [`jd_to_ymdhms`], so a date and its JD round-trip
/// across the 1582 switch.
fn calendar_jd(name: &str, args: &[Value], offset: Option<f64>) -> Result<Value, EpherError> {
    ...
    // Meeus's Gregorian/Julian switch: on and after 1582-10-15 the
    // century correction applies; before it, the Julian calendar runs
    // with b = 0 (the last Julian day, 1582-10-04, is JD 2299159.5,
    // and the next day is 1582-10-15 = JD 2299160.5).
    let b = if y * 10_000 + m * 100 + d >= 15_821_015 {
...
/// The three clock conventions the JD-to-calendar readers need, named
/// for what happens to the half-second and finer remainder:
/// `Floor` gives the calendar date that contains the moment (the date
/// never borrows from the next day); `Wrap` rounds the clock to the
/// nearest second and wraps 24:00:00 to 0:00:00 (a wall clock);
/// `Carry` rounds and lets the overflow move the date (24:00:00
/// becomes the next day's 0:00:00) - the ISO 8601 convention.
enum Round {
    Floor,
    Wrap,
    Carry,
...
/// Julian Date to Gregorian calendar date and UT clock time, the
/// standard astronomy conversion (Meeus, Astronomical Algorithms ch.
/// 7; the IAU SOFA routine jd2cal). Below JD 2299160.5 (1582-10-15)
/// the recipe yields the Julian calendar date, as those references
/// do. The instant is JD + 0.5 split into a whole day and a fraction;
/// the fraction becomes clock seconds, rounded per the convention.
fn jd_to_ymdhms(
    name: &str,
    jd: f64,
    round: Round,
) -> Result<(i64, i64, i64, i64, i64, i64), EpherError> {
    let t = jd + 0.5;
    let mut day_f = t.floor();
    let mut secs = ((t - day_f) * 86_400.0).round() as i64;
    match round {
        Round::Floor => {
            secs = 0;
        }
    ...
    if z < 0 {
        return Err(domain_error(format!(
            "{name} expects a Julian Date of 0 or more (the epoch, -4712 Jan 1.5), got {jd}"
        )));
    }
    let a = if z >= 2299161 {
    ...
    let dd = (365.25 * (c as f64)).floor() as i64;
    let e = (((b - dd) as f64) / 30.6001).floor() as i64;
    let day = b - dd - (30.6001 * (e as f64)).floor() as i64;
    let month = if e < 14 { e - 1 } else { e - 13 };
    let year = if month > 2 { c - 4716 } else { c - 4715 };
    Ok((year, month, day, secs / 3600, (secs % 3600) / 60, secs % 60))
}

/// The ISO 8601 reading of a calendar date and UT clock time:
/// `YYYY-MM-DDTHH:MM:SS`, the notation JPL Horizons and astropy print
/// and the one the guide calls the human-readable form. Years before
/// 1 carry a `-` sign (astronomical year numbering).
fn iso_string(y: i64, m: i64, d: i64, h: i64, mi: i64, s: i64) -> String {
    format!(
        "{}{:04}-{:02}-{:02}T{:02}:{:02}:{:02}",
        if y < 0 { "-" } else { "" },
        y.abs(),
