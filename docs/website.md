...
- **Navigation**: the header row is `App`, a **Docs** disclosure button
  (User guide, Examples, Scripts), Privacy, Source code; the footer Site
  column lists the same pages flat. The Docs disclosure follows the same
  APG pattern as the hamburger nav: `aria-expanded`, `hidden` panel,
...
- **Scripts page**: the repository of example scripts (`epher scripts/`,
  its README is the format standard) becomes a browsable page.
  `scripts/build-scripts.mjs` walks the folder tree (field -> area ->
  script), writes `site/scripts-data.json` (gitignored; generated in CI
  from the committed scripts) and regenerates `site/scripts.html` from
