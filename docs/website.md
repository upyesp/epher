...
- **Pages**: `/` (landing: hero, features, downloads), `/about.html`,
  `/privacy.html`, `/examples.html` — the same header/footer chrome,
  content strings under `about-*` / `privacy-*` / `ex-*` keys. The guide
  pages share the header chrome via `scripts/build-guide.mjs` (labels in
  its `CHROME` map) with the same disclosure-nav script inlined.
- **Examples page** (ADR-0036): one static page, generated by
  `scripts/build-examples.mjs` from the about.html chrome. The epher code
