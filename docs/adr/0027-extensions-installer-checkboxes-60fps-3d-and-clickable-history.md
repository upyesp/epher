...
## Amendment (2026-09-06): history files are retired; the one script extension is `.epher`
...
auto-persistence into the store, the trash, picks, and share all stay
exactly as they were.

With history files gone, scripts are the only file the app exchanges,
and they carry **one extension: `.epher`**. Save prefills change from
...
`menu-save-history`, and `history-loaded` in all eight languages. The
TUI File menu drops to three items (open script, save script, quit) and
`PromptKind`/`MenuAction` lose their History variants. The desktop save
dialog keeps no extension filter (rfd would fight typed names,
ADR-0027); the PWA download suggestion carries `.epher`. Tests pin the
three-item File menu, the `.epher` prefill, and the history pick as the
only way history moves.
