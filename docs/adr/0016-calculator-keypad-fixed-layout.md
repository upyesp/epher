...
## Amendment (2026-09-06): the TUI keypad matches the web, the file prompt edits like text, and the key-help hint tells the truth
...
which insert with a leading space; their labels carry a `·` so the
suffix form is distinguishable from the constant). The
`kp_bank_labels` mouse map grew to match. The `123` bank stays frozen
as before.

Two smaller corrections rode along with the same report:
...
- The key-help overlay (ADR-0039) always showed "Arrows scroll · q or
  Esc closes", even for banks short enough not to scroll, which made
  the arrows half a lie. The scroll half now appears only when the
