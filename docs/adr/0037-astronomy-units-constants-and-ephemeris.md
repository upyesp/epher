...
`jd()`; IAU SOFA's `jd2cal`), **`time(jd)`** returns the UT clock as
`{hour, minute, second}` rounded to the second, and **`iso(jd)`**
spells the whole instant as the ISO 8601 date-time `YYYY-MM-DDTHH:MM:SS`
that JPL Horizons and astropy print. All three implement Meeus's
recipe with its Gregorian/Julian switch: JDs from `2299160.5` read as
Gregorian dates, earlier ones as Julian calendar dates, and `iso(0)`
is the epoch itself (`-4712-01-01T12:00:00`). `time()` wraps a
rounding carry at midnight; `iso()` carries it into the next day so
the date-time stays true to the instant. JDs below `0` are outside
the calendar and raise a domain error.

The same amendment fixes `jd()`/`mjd()` to the full Meeus recipe:
dates before 1582-10-15 previously took the Gregorian century
correction (Fliegel & Van Flandern's Gregorian-only form), landing
ten days off the historical Julian-calendar JD. With the `b = 0`
Julian branch, a calendar date and its JD round-trip across the 1582
switch in both directions. No shipped script used pre-1582 dates, so
no transcript changed. The new keys join the astronomy keypad (web
and TUI, ADR-0016 amendment).
