...
the mobile hamburger, and in the TUI menu bar alike, and the TUI's
one-row file prompt only ever asks for a script. Save prefills carry
the one script extension, `.epher`.

## Amendment (2026-09-06): the file prompt's prefill is editable text
...
insert at it, a click in the input row places it, and the terminal
cursor renders it. Enter still submits, Esc still cancels.
